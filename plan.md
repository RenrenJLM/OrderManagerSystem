...
## 当前执行建议
1. Milestone 1、Milestone 2、Milestone 3 已完成，当前不再重复实现。
2. 下一步进入 Milestone 4，优先提供最小可用的订单查询区域。
3. 在查询页中先完成订单列表、订单明细查看和基础筛选，不提前扩展导出、打印、权限或网络同步。
4. 后续再在 Milestone 4 范围内补齐基础汇总结果，并继续保持轻量、可审查的改动方式。
